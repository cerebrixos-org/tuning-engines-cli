...
type DoctorRow = { level: "ok" | "warn" | "fail"; check: string; detail: string };
type HookStatus = {
  timestamp: string;
  event: string;
  cli_version: string;
  upload_status: "uploaded" | "failed" | "blocked";
  request_id?: string;
  run_id?: string;
  error?: string;
  probe?: boolean;
...
function installHook(settings: Record<string, any>, event: string, mode: HookMode, failOpen: boolean): void {
  ...
  if (event === "PreToolUse" || event === "PostToolUse" || event === "PostToolUseFailure" || event === "PermissionRequest" || event === "PermissionDenied") entry.matcher = "*";
  settings.hooks[event].push(entry);
}

function existingClaudeHookMode(settings: Record<string, any>): { mode: HookMode; failOpen: boolean } | undefined {
  for (const entries of Object.values(settings.hooks || {})) {
    for (const entry of Array.isArray(entries) ? entries : []) {
      for (const hook of Array.isArray(entry?.hooks) ? entry.hooks : []) {
        const command = String(hook?.command || "");
        if (!isGuardHookCommand(command, "claude-code")) continue;
        return {
...
function writeClaudeHooks(
  project: string,
  options: { shared?: boolean; mode?: HookMode; failOpen?: boolean; write?: boolean } = {}
): { settingsPath: string; projectDir: string; mode: HookMode; failOpen: boolean; changed: boolean; warnings: string[] } {
  ...
  for (const event of HOOK_EVENTS) installHook(settings, event, mode, failOpen);
  const changed = before !== JSON.stringify(settings);
  if (options.write !== false && changed) writeJsonFile(settingsPath, settings);
  if (options.write !== false) installClaudeGoalCommand(projectDir);
...
function installCodexHook(settings: Record<string, any>, event: string): void {
  ...
  if (event === "PreToolUse" || event === "PostToolUse" || event === "PostToolUseFailure" || event === "PermissionRequest") entry.matcher = "*";
  settings.hooks[event].push(entry);
}
...
function redactError(value: any): string {
...
function compactString(value: any, limit = 240): string | undefined {
...
function eventType(event: string): string {
  if (event === "UserPromptSubmit") return "agent.turn";
  if (event === "PermissionRequest" || event === "permission.asked") return "approval.requested";
  if (event === "PermissionDenied" || event === "approval.denied") return "approval.denied";
  if (event === "approval.approved") return "approval.approved";
  if (event === "permission.replied") return "approval.requested";
  ...
  if (event === "PreToolUse" || event === "PostToolUse" || event === "PostToolUseFailure" || event === "tool.execute.before" || event === "tool.execute.after") return "agent.tool_call";
  if (event === "SessionStart" || event === "TaskStart" || event === "TaskCreated" || event === "TaskResume" || event === "PostToolBatch" || event === "PreCompact" || event === "PostCompact" || event === "UserPromptExpansion" || event === "AfterAgent" || event === "session.created" || event === "session.idle" || event === "session.error") return "workflow.step";
...
function eventStatus(event: string, decision?: any): string {
  ...
  if (event === "PreToolUse" || event === "PermissionRequest" || event === "permission.asked" || event === "tool.execute.before") return "proposed";
  if (event === "PermissionDenied" || event === "approval.denied") return "blocked";
  if (event === "approval.approved") return "succeeded";
  if (event === "PostToolUseFailure" || event === "StopFailure" || event === "TaskCancel" || event === "session.error") return "failed";
  if (event === "PostToolUse") return "succeeded";
  if (event === "tool.execute.after") return "succeeded";
...
function statusForEvent(input: Record<string, any>, event: string, decision?: any): string {
  if (decision && decision.allowed === false) return "blocked";
  if (event === "PostToolUse" || event === "tool.execute.after") return hookFailureData(input).failed ? "failed" : "succeeded";
  if (event === "permission.replied") return permissionGranted(input) ? "succeeded" : "blocked";
  return eventStatus(event, decision);
}

function toolExecutionPhase(event: string, input: Record<string, any> = {}): string | undefined {
  if (event === "PreToolUse" || event === "tool.execute.before") return "proposed";
  if (event === "PostToolUse" || event === "tool.execute.after") return hookFailureData(input).failed ? "failed" : "executed";
  if (event === "PostToolUseFailure") return "failed";
  if (event === "PermissionRequest" || event === "permission.asked") return "proposed";
  if (event === "PermissionDenied" || event === "approval.denied") return "blocked";
  if (event === "approval.approved") return "approved";
  return undefined;
}

function permissionGranted(input: Record<string, any>): boolean {
...
function toolCallId(input: Record<string, any>, runtime: string): string | undefined {
  ...
  if (explicit) return boundedId(explicit);
...
function exitCode(input: Record<string, any>): number | undefined {
...
function hookFailureData(input: Record<string, any>): { failed: boolean; error?: string; exit_code?: number } {
  const response = toolResponse(input);
  const code = exitCode(input);
  const rawError = firstPresent(
    input.error,
    input.failure,
    input.exception,
    input.error_message,
...
function previewForInput(input: Record<string, any>): string | undefined {
...
function humanSummary(input: Record<string, any>, event: string, runtime: string): string | undefined {
  ...
  if (event === "PermissionRequest" || event === "permission.asked") return "Approval requested.";
  if (event === "PermissionDenied") return "Approval denied.";
  if (event === "permission.replied") return permissionGranted(input) ? "OpenCode permission approved." : "OpenCode permission denied.";
  if (event === "SubagentStart") return "Started a delegated task.";
  if (event === "SubagentStop") return failure.failed ? "Delegated task failed." : "Delegated task completed.";
  if (event === "Stop") return failure.failed ? "Agent response failed." : "Agent response completed.";
  if (event === "SessionEnd") return failure.failed ? `${traceRuntimeLabel(runtime)} session failed.` : `${traceRuntimeLabel(runtime)} session completed.`;
  if (event === "PreToolUse" || event === "tool.execute.before") return operationSummary(operationClass(input), "proposed");
...
function operationClass(input: Record<string, any>): string {
  ...
  if (/read|view|cat/.test(name)) return "file_read";
  if (/write|edit|patch|notebook/.test(name)) return "file_change";
  if (/grep|glob|search|find/.test(name) || /(^|\s)(rg|grep|find)(\s|$)/.test(command)) return "repository_search";
  if (/^git\b/.test(command)) return "git_inspection";
  if (/(^|\s)(pytest|rspec|rails test|npm test|npm run test|yarn test|pnpm test|go test|cargo test)(\s|$)/.test(command)) return "test_runner";
  if (/(^|\s)(curl|wget|httpie)(\s|$)/.test(command) || /http|fetch|request/.test(name)) return "http_client";
  if (/bash|shell|terminal|command|exec/.test(name)) return "shell_command";
  return "tool_operation";
}

function operationSummary(operation: string, status: "proposed" | "succeeded" | "failed"): string {
  ...
  if (status === "proposed") return `${phrase}.`;
  return `${phrase} ${status === "failed" ? "unsuccessfully" : "successfully"}.`;
}

function safeRelativePath(value: any, cwd?: any): string | undefined {
  const raw = String(value || "").trim();
  if (!raw || raw.includes("\0") || raw.includes("\n") || raw.startsWith("gAAAA")) return undefined;
  const redacted = redact(raw);
  if (redacted.includes("[FILTERED]")) return undefined;
  const normalized = redacted.replace(/\\/g, "/");
  if (!path.isAbsolute(raw)) return normalized.replace(/^\.\//, "").slice(0, 180);
  const base = cwd ? path.resolve(String(cwd)) : undefined;
  const relative = base ? path.relative(base, raw).replace(/\\/g, "/") : "";
  if (relative && !relative.startsWith("../") && relative !== "..") return relative.slice(0, 180);
  return path.basename(raw).slice(0, 180);
}

function toolPath(input: Record<string, any>): string | undefined {
  const values = toolInput(input) || {};
  return safeRelativePath(
    firstPresent(values.file_path, values.filePath, values.path, values.filename, values.target_file, values.targetFile),
    input.cwd
  );
}

function friendlyToolLabel(value: string): string {
  if (/^(bash|shell|exec_command|write_stdin)$/i.test(value)) return "Shell command";
  if (/^(apply_patch|edit|write)$/i.test(value)) return "File update";
  if (/^(read|read_file|view)$/i.test(value)) return "File read";
...
function safeToolSummary(input: Record<string, any>, completed: boolean): string {
  ...
  if (/^(read|read_file|view)$/i.test(name) && file) return `Read ${file}`;
  if (/^(write|create|write_file)$/i.test(name) && file) return `${completed ? "Created" : "Create"} ${file}`;
  if (/^(edit|apply_patch|update_file)$/i.test(name) && file) return `${completed ? "Updated" : "Update"} ${file}`;
  ...
  if (completed && safeResponse) return `${friendlyToolLabel(name)} completed: ${safeResponse}`;
  if (safeInput) return `${completed ? friendlyToolLabel(name) : `Run ${friendlyToolLabel(name)}`}: ${safeInput}`;
  return completed ? `${friendlyToolLabel(name)} completed.` : `Run ${friendlyToolLabel(name)}.`;
}

function subagentTaskName(input: Record<string, any>): string | undefined {
  const values = toolInput(input) || {};
  return compactString(firstPresent(
    input.task_name,
    input.taskName,
...
function subagentResultSummary(input: Record<string, any>): string | undefined {
  return compactString(firstPresent(
    input.result_summary,
    input.last_assistant_message,
    input.response_preview,
    toolResponse(input)?.summary
  ), 180);
}
...
function nativeTaskSeed(input: Record<string, any>, event: string, runtime: string): string {
  ...
  if (TOOL_LIFECYCLE_EVENTS.has(event) && callId) return `${runtime}:tool:${callId}`;
  const tool = toolName(input);
...
function nativeParentTaskSeed(input: Record<string, any>, event: string, runtime: string): string | undefined {
  ...
  if (event === "SessionStart" || event === "UserPromptSubmit") return undefined;
  if (event === "PostToolUse" || event === "PostToolUseFailure" || event === "tool.execute.after") return nativeTaskSeed(input, "PreToolUse", runtime);
...
function nativeEventId(input: Record<string, any>, event: string, runtime: string): string {
  const callId = toolCallId(input, runtime);
  if (TOOL_LIFECYCLE_EVENTS.has(event) && callId) return `evt_${sha([runtime, "tool", callId].join(":")).slice(0, 24)}`;
...
function nativeParentEventId(input: Record<string, any>, event: string, runtime: string): string | undefined {
  ...
  if (input.root_event_id) return String(input.root_event_id);
...
type NativeTurnState = {
  ...
  tool_spans: Record<string, string>;
  updated_at: string;
};
...
function removeHeader(headers: string[], name: string): void {
  const prefix = `${name.toLowerCase()}:`;
  for (let index = headers.length - 1; index >= 0; index -= 1) {
    if (headers[index].toLowerCase().startsWith(prefix)) headers.splice(index, 1);
  }
}
...
function claudeStatusPath(projectDir: string): string {
  return path.join(projectDir, ".claude", "tuning-engines-hook-status.jsonl");
}

function appendClaudeHookStatus(projectDir: string, input: Record<string, any>, event: string, uploadStatus: HookStatus["upload_status"], error?: any): void {
  try {
...
function appendClaudeHookStatusForInput(input: Record<string, any>, event: string, uploadStatus: HookStatus["upload_status"], error?: any): void {
...
function recentClaudeHookStatuses(projectDir: string, limit = 5): HookStatus[] {
  try {
    ...
    return fs.readFileSync(statusPath, "utf-8")
      ...
      .filter(Boolean)
      .slice(-limit)
      .map((line) => JSON.parse(line) as HookStatus);
  } catch {
    return [];
  }
}

function installedHookCommands(settings: Record<string, any>, event: string): string[] {
...
function probePayload(projectDir: string, event: string, ids: { runId: string; requestId: string; sessionId: string }): Record<string, any> {
  return compact({
    te_probe: true,
...
function traceContainsProbeEvents(trace: any, requiredEvents: string[], runId: string, requestId: string): { found: string[]; eventCount: number } {
...
async function waitForProbeTrace(client: TuningEnginesClient, runId: string, requestId: string, requiredEvents: string[]): Promise<{ ok: boolean; detail: string; trace?: any }> {
  let lastDetail = "";
...
async function runClaudeDoctorProbe(
  client: TuningEnginesClient,
  projectDir: string,
  shared = false
): Promise<{ ok: boolean; run_id: string; request_id: string; commands: Array<Record<string, any>>; server: Record<string, any> }> {
...
function installOpenCode(projectDir: string): string[] {
  ...
  config.plugin = config.plugin.filter((entry: any) => String(entry) !== "opencode-helicone-session");
        ...
        if (opts.probe) {
          ...
          if (!ok) {
            ...
            for (const command of probe.commands) {
              ...
              if (!command.ok && command.stderr) console.log(`  ${command.stderr}`);
            }
  ...
  claude
    ...
    .action(async (opts) => {
      ...
      try {
        ...
        if (event !== "SessionStart") await recordTrace(client, input, event, decision);
