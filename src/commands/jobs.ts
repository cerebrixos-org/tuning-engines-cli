  ...
  jobs
    ...
    .action(async (opts) => {
      try {
        ...
        output.table(
          ["ID", "Status", "Base Model", "Output", "GPU Min", "Cost", "Created"],
          jobs.map((j: any) => [
            j.id,
            j.status,
            j.base_model || "-",
  ...
  jobs
    ...
    .action(async (opts) => {
      try {
        ...
        if (opts.repoUrl) params.repo_url = opts.repoUrl;
        if (opts.branch) params.branch = opts.branch;
        if (opts.githubToken) params.github_token = opts.githubToken;
        if (opts.epochs) params.num_epochs = parseInt(opts.epochs);
        if (opts.maxExamples) params.max_examples = parseInt(opts.maxExamples);
        if (opts.baseUserModelId) params.base_user_model_id = opts.baseUserModelId;
        if (opts.s3Bucket) params.s3_output_bucket = opts.s3Bucket;
        if (opts.s3Key) params.s3_access_key_id = opts.s3Key;
        if (opts.s3Secret) params.s3_secret_access_key = opts.s3Secret;
        if (opts.s3Region) params.s3_region = opts.s3Region;
        if (opts.agent) params.agent = opts.agent;
        if (opts.qualityTier) params.quality_tier = opts.qualityTier;
  ...
  jobs
    ...
    .action(async (opts) => {
      try {
        ...
        if (opts.baseUserModelId) params.base_user_model_id = opts.baseUserModelId;
        if (opts.maxExamples) params.max_examples = parseInt(opts.maxExamples);
        if (opts.repoSizeMb) params.repo_size_mb = parseFloat(opts.repoSizeMb);
