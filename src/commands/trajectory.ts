...
export function registerTrajectoryCommands(program: Command, getClient: () => TuningEnginesClient): void {
  ...
  studies.command("show <id>")
    .action(async (id: string) => output.json(await getClient().getComparisonStudy(id)));
  ...
  studies.command("update <id>")
    ...
    .action(async (id: string, opts) => output.json(await getClient().updateComparisonStudy(id, parseObject(opts.data) || {})));
  studies.command("run <id>")
    .action(async (id: string) => output.json(await getClient().runComparisonStudy(id)));
