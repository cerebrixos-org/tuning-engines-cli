...
function nodePackageJson(framework: Framework): string {
  ...
  if (framework === "restate") deps["@restatedev/restate-sdk"] = "^1.14.4";
  if (framework === "dbos") deps["@dbos-inc/dbos-sdk"] = "^4.0.0";
  if (framework === "dapr") deps["@dapr/dapr"] = "^3.5.0";
...
function pythonWorkflowFiles(
  framework: Framework,
  source: string,
  entrypointPath = "workflow.py",
): Record<string, string> {
...
function pythonReadme(framework: Framework, entrypointPath: string, helperPath: string): string {
  const label = FRAMEWORK_LABELS[framework];
  return sharedReadme(framework) + `
## Install
...
function pythonRequirements(framework: Framework): string {
  const lines = ["httpx>=0.27"];
  if (framework === "prefect") lines.push("prefect>=3.0");
  if (framework === "dagster") lines.push("dagster>=1.8");
  if (framework === "airflow") lines.push("apache-airflow>=2.10");
  return `${lines.join("\n")}\n`;
}
...
function tuningEnginesPythonHelper(): string {
  return `import os
import time
import uuid
