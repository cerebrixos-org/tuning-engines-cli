...
exports.resolveInferenceBearer = resolveInferenceBearer;
exports.callInference = callInference;
...
function parseJsonObject(raw, label = "payload") {
...
async function resolveInferenceBearer(client, explicitKey) {
    ...
    if (!token) {
        throw new Error("Could not obtain an inference credential. Set TE_INFERENCE_KEY or pass --key.");
    }
    return String(token);
}
async function callInference(endpoint, payload, bearer, options) {
    const normalized = endpoint.startsWith("/") ? endpoint : `/${endpoint}`;
    const isResourceEndpoint = normalized.startsWith("/agents/") && normalized.endsWith("/message") ||
...
//# sourceMappingURL=inference_request.js.map